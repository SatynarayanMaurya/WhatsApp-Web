...
import IndividualChatSkeleton from '../pages/IndividualChatSkeleton'
import ChatHeader from './ChatHeader'
import socket from '../Utils/socket'
import { setBotHistory } from '../Redux/Slices/userSlice'
function AiChatPage() {
    ...
    const botHistory = useSelector((state)=>state.user.botHistory)
  ...
  }, [botHistory]); // Runs when opening a chat
  ...
  useEffect(() => {
    ...
    const sendMessage = () => {
        ...
        socket.emit("sendMessage", {
            chatId: "ai_chat_1",  // ID of this chat, can be any string for AI
            sender: userDetails?._id,       // The current user ID
            receiver: "AI_BOT",   // This is what backend checks for AI responses
            message: message,
            type: "text",
            timestamp:Date.now()
        ...
        setMessage("");
        dispatch(setBotHistory({data}))
    };

    // Listen for AI bot reply
    useEffect(() => {
